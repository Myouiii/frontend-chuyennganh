...
function FilterResult() {
  ...
  function renderFilterOption(type) {
    ...
    return (
      list &&
      list.map((item, index) => (
        <div key={index} className="Filter-Options-item my-3">
  ...
  return (
    <div className="container" style={{ minHeight: '100vh' }}>
      ...
      {/* Số  kết quả tìm kiếm */}
      {!isLoading && (
        <h2 className="font-size-24px mb-3">
      ...
      {isLoading ? (
        ...
        <>
          {/* Bộ lọc  */}
          <div className="Filter-Options py-4 bg-white rounded-lg box-sha-home">
            <div className="list-active px-4 pb-2 flex justify-between">
              <b className="font-size-22px filter-list mr-5">Bộ lọc</b>
              {tagList.length > 0 && (
                <>
            ...
            <div className="px-4 pt-4">{renderFilterOption(type)}</div>
          </div>
