...
function Description(props) {
  ...
  return (
    <Row className="Product-Desc bg-white p-2" id="descId">
      {/* Bài viết chi tiết */}
      <Col
        span={24}
        md={16}
        className={`p-2 ${!isHideDesc ? 'hide-desc' : ''}`}>
        <h2 className="font-bold text-[25px]">Mô tả sản phẩm</h2>
        <div className="underline-title"></div>
        <Posts content={desc} />
      </Col>

      {/* Thông số kỹ thuật */}
      <Col span={24} md={8} className={`p-2 ${!isHideDesc ? 'hide-desc' : ''}`}>
      ...
      {isShowSeeMore && (
        <h3
          className="block mx-auto py-4 see-more ease-trans"
          onClick={onSeeMore}>
